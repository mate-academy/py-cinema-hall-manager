...
class ActorManager:
    ...
    def create(self, first_name: str, last_name: str):
        query = "INSERT INTO actor_manager (first_name, last_name) VALUES (?, ?)"
        self.cursor.execute(query, (first_name, last_name))
        self.conn.commit()
        return self.cursor.lastrowid

    def all(self):
        query = "SELECT id, first_name, last_name FROM actor_manager"
        self.cursor.execute(query)
        results = self.cursor.fetchall()
        return [Actor(id=row[0], first_name=row[1], last_name=row[2]) for row in results]

    def update(self, actor_id: int, first_name: str, last_name: str):
        query = "UPDATE actor_manager SET first_name = ?, last_name = ? WHERE id = ?"
