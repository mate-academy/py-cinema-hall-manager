...
class ActorManager:
    ...
    def all(self):
        self.actors_cursor = self._connection.execute(f"SELECT * FROM {self.table_name}")
        return [Actor(*row) for row in self.actors_cursor]

    def update(self, id_to_update: int, new_first_name: str, new_lat_name: str) -> None:
        self._connection.execute(
            f"UPDATE {self.table_name} "
            f"SET first_name=?, last_name=? "
            f"WHERE id=?",
            (new_first_name, new_lat_name, id_to_update)
        )
        self._connection.commit()
